...
Consider a simple Bayesian model where the likelihood is a mixture of normals model:
```{r}
...
We'll try using the random walk Metropolis-Hastings sampler for this model - we seem to explore one mode well but the other has not been explored at all
...
- posterior effectively flattened or softened at higher temperatures
- allows the "hot" chains to move more freely across parameter space 
- periodically, the states of the chains are swapped according to an acceptance rule that preserves the correct stationary distribution
- hopefully higher-temperature chains will quickly explore multiple modes and then swap states with the lower-temperature chains
